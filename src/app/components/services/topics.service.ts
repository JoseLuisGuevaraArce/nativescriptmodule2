...
export class TopicsService {
  api: string = 'https://561a-177-222-108-106.ngrok-free.app';
  ...
  getDb(fnOk, fnError) {
    return new sqlite('My db logs', (err, db) => {
      if (err) {
        console.error('Error to open DB!', err);
      } else {
        console.log('Opened DB: ', db.isOpen() ? 'Yes' : 'No');
  ...
  getTopics() {
    return getJSON(this.api + '/getAll');
  }
...
// }
  ...
  // addTopic(topic: TopicModel): void {
  //   this.topics.push(topic);
  // }

  // getTopicById(id: number): TopicModel | undefined {
  //   return this.topics.find(topic => topic.id === id) || undefined
  // }
// }